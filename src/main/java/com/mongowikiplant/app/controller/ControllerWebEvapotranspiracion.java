...
import com.mongowikiplant.app.entity.Evapotranspiracion.MesCantidad;
...
@Controller
@RequestMapping("/evapotranspiracion")
public class ControllerWebEvapotranspiracion {
    ...
    @GetMapping("/crear")
    public String evapotranspiracionCrearTemplate(Model model) {
        model.addAttribute("evapotranspiracion", new Evapotranspiracion());
        return "evapotranspiracion-form";
    }

    @GetMapping("/lista")
    public String evapotranspiracionListTemplate(Model model) {
        List<Evapotranspiracion> evapotranspiraciones = evapotranspiracionRepository.findAll();

        for (Evapotranspiracion evapotranspiracion : evapotranspiraciones) {
            Map<String, Double> mesCantidadMap = new HashMap<>();
            for (Evapotranspiracion.MesCantidad registro : evapotranspiracion.getRegistros()) {
                mesCantidadMap.put(registro.getMes(), registro.getCantidad());
            }
            evapotranspiracion.setMesCantidadMap(mesCantidadMap);
        }

        model.addAttribute("evapotranspiraciones", evapotranspiraciones);
    ...
    public String evapotranspiracionEditTemplate(@PathVariable("id") String id, Model model) {
        Evapotranspiracion evapotranspiracion = evapotranspiracionRepository.findById(id)
                .orElseThrow(() -> new NotFoundException("Evapotranspiracion no encontrada"));
    ...
    @PostMapping("/save")
    public String evapotranspiracionSaveProcess(
            @ModelAttribute("evapotranspiracion") Evapotranspiracion evapotranspiracion,
        ...
        if (evapotranspiracion.getId() == null || evapotranspiracion.getId().isEmpty()) {
            evapotranspiracion.setId(null);
        }

        if (fecha != 0) {
            evapotranspiracion.setFecha(fecha);
        }
        ...
        evapotranspiracion.setRegistros(registros);
