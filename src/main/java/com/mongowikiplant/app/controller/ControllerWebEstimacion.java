...
import com.mongowikiplant.app.entity.Estimacion.MesCantidad;
...
public class ControllerWebEstimacion {
    ...
    @GetMapping("/crear")
    public String estimacionCrearTemplate(Model model) {
        model.addAttribute("estimacion", new Estimacion());
        return "estimacion-form";
    }

    @GetMapping("/lista")
    public String estimacionListTemplate(Model model) {
        ...
        for (Estimacion estimacion : estimaciones) {
            ...
            for (Estimacion.MesCantidad registro : estimacion.getRegistros()) {
                mesCantidadMap.put(registro.getMes(), registro.getCantidad());
            }
            estimacion.setMesCantidadMap(mesCantidadMap);
        }

        model.addAttribute("estimaciones", estimaciones);
    ...
    public String estimacionEditTemplate(@PathVariable("id") String id, Model model) {
        Estimacion estimacion = estimacionRepository.findById(id)
                .orElseThrow(() -> new NotFoundException("Estimacion no encontrada"));
    ...
    @PostMapping("/save")
    public String estimacionSaveProcess(
            @ModelAttribute("estimacion") Estimacion estimacion,
        ...
        estimacion.setRegistros(registros);
